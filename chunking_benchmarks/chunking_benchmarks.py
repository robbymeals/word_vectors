...
if __name__ == '__main__':
    ...
    iob_train_vecs = np.array([np.vstack([get_word_vec(iob_train[j][i])
        for i in range(len(iob_train[j]))]) for j in range(len(iob_train))])
    iob_test_vecs = np.array([np.vstack([get_word_vec(iob_test[j][i])
    ...
    y_train_flat = list(chain(* [[tok[2] for tok in sentence] 
        for sentence in iob_train[0:subset]]))
